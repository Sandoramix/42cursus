...
ScavTrap::ScavTrap() : ClapTrap(), guardGateStatus(DISABLED)
{
	...
	std::cout << "[SP][DEFAULT]\tconstructor\tname: " << this->name << std::endl;
}
...
ScavTrap::ScavTrap(ScavTrap &c) : ClapTrap(c), guardGateStatus(DISABLED)
{
	...
	this->energyPoints = 50;
	this->attackDamage = 20;
	std::cout << "[SP][COPY]\tconstructor\tname: " << this->name << std::endl;
}
...
ScavTrap::ScavTrap(std::string name) : ClapTrap(name), guardGateStatus(DISABLED)
{
	...
	this->energyPoints = 50;
	this->attackDamage = 20;
	std::cout << "[SP]\t\tconstructor\tname: " << this->name << std::endl;

}

ScavTrap::~ScavTrap()
{
	std::cout << "[SP]\t\tdestructor\tname: " << this->name << std::endl;
}
...
void ScavTrap::guardGate(void)
{
	...
	if (this->guardGateStatus == DISABLED){
		std::cout << "[SP]\tScavTrap " << this->name << " Turned off GUARD GATE. Everybody should rest eventually." << std::endl;
	} else {
		std::cout << "[SP]\tScavTrap " << this->name << "Turned on GUARD GATE. GLHF" << std::endl;
	}
}
